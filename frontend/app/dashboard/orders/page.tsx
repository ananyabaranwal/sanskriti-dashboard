...
function Toast({ msg, type, onClose }: { msg:string; type:"success"|"error"; onClose:()=>void }) {
  ...
  return (
    <div style={{position:"fixed",top:"24px",right:"24px",zIndex:9999,padding:"12px 16px",borderRadius:"12px",background:type==="success"?"#F0FDF4":"#FEF2F2",border:`1px solid ${type==="success"?"#BBF7D0":"#FECACA"}`,color:type==="success"?"#15803d":"#dc2626",fontSize:"13px",fontWeight:500,boxShadow:"0 6px 24px rgba(0,0,0,.1)",display:"flex",alignItems:"center",gap:"10px",maxWidth:"340px",animation:"slideDown .3s ease",fontFamily:"inherit"}}>
...
function SkeletonRow() {
  return (
    <div style={{display:"grid",gridTemplateColumns:"150px 1fr 100px 90px 110px 72px",padding:"14px 18px",borderBottom:"1px solid #F5EDE0",alignItems:"center",gap:"0"}}>
...
function CreateOrderModal({ onClose, onSuccess }: { onClose:()=>void; onSuccess:()=>void }) {
  ...
  return (
    <div onClick={(e)=>{if(e.target===e.currentTarget)onClose();}} style={{position:"fixed",inset:0,background:"rgba(44,24,16,.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:2000,padding:"20px",backdropFilter:"blur(6px)"}}>
      <div style={{background:"#FFFDF9",borderRadius:"20px",width:"100%",maxWidth:"520px",maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 64px rgba(44,24,16,.4)",animation:"scaleIn .3s ease"}}>
        <div style={{background:"linear-gradient(135deg,#1A0F0A,#2C1810,#3D2B1F)",padding:"22px 28px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",borderRadius:"20px 20px 0 0"}}>
          <div>
            <h2 style={{fontSize:"19px",fontFamily:"Georgia,serif",color:"#F5E6C8",fontWeight:400}}>Create New Order</h2>
            <p style={{fontSize:"12px",color:"rgba(245,230,200,.5)",marginTop:"2px"}}>Fill buyer and item details</p>
          </div>
          <button onClick={onClose} style={{width:"30px",height:"30px",borderRadius:"50%",background:"rgba(255,255,255,.08)",border:"none",color:"#C9A84C",fontSize:"18px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"inherit"}}>×</button>
        </div>
        <div style={{padding:"24px 28px",display:"flex",flexDirection:"column",gap:"14px"}}>
          <p style={{fontSize:"11px",fontWeight:700,color:"#8B6914",textTransform:"uppercase",letterSpacing:".06em"}}>Buyer Information</p>
          ...
          <p style={{fontSize:"11px",fontWeight:700,color:"#8B6914",textTransform:"uppercase",letterSpacing:".06em",marginTop:"4px"}}>Item Details</p>
          ...
          <div style={{display:"flex",gap:"10px"}}>
            <button onClick={onClose} style={{flex:1,padding:"12px",borderRadius:"8px",border:"1.5px solid #E8D5A3",background:"transparent",color:"#6B4F12",fontSize:"14px",fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Cancel</button>
            <button onClick={submit} disabled={loading} style={{flex:2,padding:"12px",borderRadius:"8px",background:loading?"#E8D5A3":"linear-gradient(135deg,#C9A84C,#8B6914)",color:loading?"#A08060":"#2C1810",border:"none",fontSize:"14px",fontWeight:700,cursor:loading?"not-allowed":"pointer",fontFamily:"inherit"}}>
...
function OrderDrawer({ order, onClose, onStatusUpdate, onGenerateBill }: {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:1000}}>
      ...
      {/* Drawer */}
      <div style={{position:"absolute",right:0,top:0,bottom:0,width:"440px",background:"#FFFDF9",boxShadow:"-8px 0 48px rgba(44,24,16,.2)",overflowY:"auto",animation:"slideLeft .3s ease",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{padding:"20px 24px",flex:1,display:"flex",flexDirection:"column",gap:"18px"}}>
          ...
          {/* Buyer */}
          <div>
            <p style={{fontSize:"10px",fontWeight:700,color:"#A08060",textTransform:"uppercase",letterSpacing:".08em",marginBottom:"8px"}}>Buyer Details</p>
            <div style={{background:"#FBF7F0",borderRadius:"10px",padding:"13px 15px",border:"1px solid #E8D5A3"}}>
              <div style={{fontSize:"15px",fontWeight:600,color:"#2C1810"}}>{order.buyer?.name}</div>
              <div style={{fontSize:"13px",color:"#6B4F12",marginTop:"3px"}}>{order.buyer?.phone}</div>
              {order.buyer?.email&&<div style={{fontSize:"13px",color:"#A08060",marginTop:"2px"}}>{order.buyer.email}</div>}
              {order.buyer?.address?.city&&(
                <div style={{fontSize:"12px",color:"#A08060",marginTop:"5px",display:"flex",alignItems:"center",gap:"5px"}}>
                  <span>📍</span>
                  <span>{[order.buyer.address.street,order.buyer.address.city,order.buyer.address.state,order.buyer.address.pincode].filter(Boolean).join(", ")}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Items */}
          <div>
            <p style={{fontSize:"10px",fontWeight:700,color:"#A08060",textTransform:"uppercase",letterSpacing:".08em",marginBottom:"8px"}}>Items ({order.items?.length})</p>
            <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
              {order.items?.map((item:any,i:number)=>(
                <div key={item._id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:i%2===0?"#FBF7F0":"#FFFDF9",borderRadius:"8px",border:"1px solid #F0E4C0"}}>
                  <div>
                    <div style={{fontSize:"13px",fontWeight:500,color:"#2C1810"}}>{item.name}</div>
                    {item.description&&<div style={{fontSize:"11px",color:"#A08060",marginTop:"2px"}}>{item.description}</div>}
                    <div style={{fontSize:"12px",color:"#6B4F12",marginTop:"3px"}}>Qty {item.quantity} × ₹{item.price?.toLocaleString("en-IN")}</div>
                  </div>
                  <div style={{fontWeight:700,color:"#2C1810",fontSize:"14px",fontFamily:"Georgia,serif"}}>₹{item.total?.toLocaleString("en-IN")}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{background:"#FBF7F0",borderRadius:"10px",padding:"13px 15px",border:"1px solid #E8D5A3"}}>
            ...
            {order.discountAmount>0&&<div style={{display:"flex",justifyContent:"space-between",fontSize:"13px",color:"#6B4F12",marginBottom:"5px"}}><span>Discount</span><span>-₹{order.discountAmount?.toLocaleString("en-IN")}</span></div>}
            <div style={{display:"flex",justifyContent:"space-between",fontSize:"16px",fontWeight:700,borderTop:"1px solid #E8D5A3",paddingTop:"9px",marginTop:"5px"}}>
              <span style={{color:"#2C1810"}}>Total</span>
              <span style={{color:"#8B6914",fontFamily:"Georgia,serif"}}>₹{order.total?.toLocaleString("en-IN")}</span>
            </div>
            <div style={{fontSize:"12px",color:"#A08060",marginTop:"6px",display:"flex",justifyContent:"space-between"}}>
              <span>Payment: <strong style={{color:"#6B4F12"}}>{order.paymentMethod}</strong></span>
              <span style={{padding:"2px 8px",borderRadius:"99px",background:order.paymentStatus==="PAID"?"#F0FDF4":"#FFFBEB",color:order.paymentStatus==="PAID"?"#15803d":"#854d0e",fontSize:"11px",fontWeight:700,border:`1px solid ${order.paymentStatus==="PAID"?"#BBF7D0":"#FDE68A"}`}}>{order.paymentStatus}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{display:"flex",gap:"6px",flexWrap:"wrap"}}>
              {STATUS_FLOW.map((s)=>{
                ...
                return (
                  <button key={s} disabled={isPast||isCurrent||updating} onClick={()=>handleStatus(s)} style={{padding:"6px 12px",borderRadius:"99px",fontSize:"11px",fontWeight:600,border:`1px solid ${isCurrent?ss.border:isPast?"#E8D5A3":"#E8D5A3"}`,background:isCurrent?ss.bg:isPast?"#F5EDE0":"transparent",color:isCurrent?ss.color:isPast?"#C4A882":"#6B4F12",cursor:isPast||isCurrent?"default":"pointer",transition:"all .15s",opacity:updating?.6:1}}>
                ...
                {[...order.statusHistory].reverse().slice(0,5).map((h:any,i:number)=>{
                  ...
                  return (
                    <div key={i} style={{display:"flex",gap:"10px",alignItems:"flex-start",marginBottom:"12px",position:"relative"}}>
                      <div style={{width:"12px",height:"12px",borderRadius:"50%",background:hs.dot,flexShrink:0,position:"absolute",left:"-17px",top:"2px",border:"2px solid #FFFDF9"}}/>
                      <div>
        ...
        {/* Drawer footer — Generate Invoice */}
        <div style={{padding:"16px 24px",borderTop:"1px solid #F0E4C0",flexShrink:0,background:"#FFFDF9"}}>
          <button onClick={handleBill} disabled={billing} style={{width:"100%",padding:"13px",borderRadius:"10px",background:billing?"#E8D5A3":"linear-gradient(135deg,#C9A84C,#8B6914)",color:billing?"#A08060":"#2C1810",border:"none",fontSize:"14px",fontWeight:700,cursor:billing?"not-allowed":"pointer",fontFamily:"inherit",letterSpacing:".02em",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px"}}>
            {billing?<><span style={{width:"14px",height:"14px",borderRadius:"50%",border:"2px solid rgba(44,24,16,.3)",borderTopColor:"#2C1810",animation:"spin .7s linear infinite",display:"inline-block"}}/>Generating...</>:<>🧾 Generate GST Invoice</>}
          </button>
        </div>
      </div>
    </div>
    ...
    <div style={{maxWidth:"1100px"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"24px",flexWrap:"wrap",gap:"12px"}}>
        <div>
          ...
          <h1 style={{fontSize:"26px",fontFamily:"Georgia,serif",color:"#2C1810",fontWeight:400}}>Orders</h1>
          <p style={{fontSize:"14px",color:"#A08060",marginTop:"3px"}}>{total} total orders</p>
        </div>
        <button onClick={()=>setShowCreate(true)} style={{padding:"11px 22px",borderRadius:"9px",background:"linear-gradient(135deg,#C9A84C,#8B6914)",color:"#2C1810",border:"none",fontSize:"13px",fontWeight:700,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 14px rgba(201,168,76,.25)",letterSpacing:".02em"}}>
          + New Order
        </button>
      </div>
        ...
        {loading?Array(5).fill(0).map((_,i)=><div key={i} className="skeleton" style={{height:"90px",borderRadius:"12px"}}/>):(
          <>
            ...
            {[
              ...
              return (
                <div key={s} onClick={()=>{setStatusFilter(statusFilter===s?"":s);fetchOrders(1,search,statusFilter===s?"":s,true);}} style={{background:"#FFFDF9",border:`1px solid ${statusFilter===s?sc.border:"#E8D5A3"}`,borderRadius:"14px",padding:"16px 18px",cursor:"pointer",transition:"all .2s"}}>
                  <div style={{fontSize:"18px",marginBottom:"6px"}}>{icon}</div>
                  <div style={{fontSize:"20px",fontWeight:700,color:sc.color,fontFamily:"Georgia,serif"}}>{getCount(s)}</div>
                  <div style={{fontSize:"11px",color:"#A08060",marginTop:"3px"}}>{label}</div>
                </div>
              );
            })}
          </>
      ...
      <form onSubmit={handleSearch} style={{display:"flex",gap:"10px",marginBottom:"16px",flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:"200px",position:"relative"}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by order no., buyer name, phone..." style={{width:"100%",padding:"10px 14px 10px 36px",borderRadius:"8px",border:"1.5px solid #E8D5A3",fontSize:"13px",color:"#2C1810",background:"#FFFDF9",outline:"none",fontFamily:"inherit"}}/>
        </div>
        <select value={statusFilter} onChange={e=>{setStatusFilter(e.target.value);fetchOrders(1,search,e.target.value,true);}} style={{padding:"10px 14px",borderRadius:"8px",border:"1.5px solid #E8D5A3",fontSize:"13px",color:"#2C1810",background:"#FFFDF9",outline:"none",cursor:"pointer",fontFamily:"inherit"}}>
          <option value="">All Status</option>
          {ALL_STATUSES.map(s=><option key={s} value={s}>{s}</option>)}
        </select>
        <button type="submit" style={{padding:"10px 20px",borderRadius:"8px",background:"#2C1810",color:"#C9A84C",border:"none",fontSize:"13px",fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Search</button>
        {(search||statusFilter)&&(
          <button type="button" onClick={()=>{setSearch("");setStatusFilter("");fetchOrders(1,"","",true);}} style={{padding:"10px 16px",borderRadius:"8px",border:"1px solid #E8D5A3",background:"transparent",color:"#A08060",fontSize:"13px",cursor:"pointer",fontFamily:"inherit"}}>Clear</button>
        )}
      </form>
      ...
      <div style={{background:"#FFFDF9",border:"1px solid #E8D5A3",borderRadius:"16px",overflow:"hidden",opacity:tableLoading?.6:1,transition:"opacity .25s"}}>
        ...
        {/* Table header */}
        <div style={{display:"grid",gridTemplateColumns:"150px 1fr 100px 90px 110px 72px",padding:"10px 18px",background:"linear-gradient(135deg,#2C1810,#3D2B1F)",borderBottom:"1px solid #E8D5A3"}}>
        ...
        {loading?(
          ...
          <div style={{padding:"64px 20px",textAlign:"center"}}>
            ...
            <p style={{fontSize:"13px",color:"#A08060",marginBottom:"20px"}}>{search||statusFilter?"Try different filters or clear search":"Create your first order to get started"}</p>
            {!search&&!statusFilter&&<button onClick={()=>setShowCreate(true)} style={{padding:"10px 24px",borderRadius:"8px",background:"linear-gradient(135deg,#C9A84C,#8B6914)",color:"#2C1810",border:"none",fontSize:"13px",fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>+ Create First Order</button>}
          </div>
        ):(
          <>
            {orders.map((o,i)=>{
              ...
              return (
                <div key={o._id} onClick={()=>setSelectedOrder(o)} style={{display:"grid",gridTemplateColumns:"150px 1fr 100px 90px 110px 72px",padding:"14px 18px",borderBottom:i<orders.length-1?"1px solid #F5EDE0":"none",background:i%2===0?"#FFFDF9":"#FDFAF4",alignItems:"center",cursor:"pointer",transition:"background .15s"}}
                ...
                >
                  ...
                  <div>
                    <span style={{display:"inline-flex",alignItems:"center",gap:"5px",padding:"3px 9px",borderRadius:"99px",fontSize:"10px",fontWeight:700,background:sc.bg,color:sc.color,border:`1px solid ${sc.border}`}}>
