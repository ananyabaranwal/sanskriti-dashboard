...
function StatCard({ icon, label, value, sub, color, href }: {
  ...
  const inner = (
    <div style={{ background: "#FFFDF9", border: "1px solid #E8D5A3", borderRadius: "16px", padding: "20px", display: "flex", flexDirection: "column", gap: "8px", transition: "box-shadow .2s", cursor: href ? "pointer" : "default" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {href && <span style={{ color: "#C9A84C", fontSize: "14px", opacity: 0.5 }}>→</span>}
      </div>
      <div style={{ fontSize: "22px", fontWeight: "700", color: "#2C1810", fontFamily: "Georgia, serif" }}>{value}</div>
      <div style={{ fontSize: "12px", color: "#A08060" }}>{label}</div>
      {sub && <div style={{ fontSize: "11px", color: "#C4A882" }}>{sub}</div>}
    </div>
  );
  ...
  return (
    <Link href={href} style={{ textDecoration: "none" }}>
      <div
        ...
        style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 12px", borderRadius: "10px", background: hovered ? color : "transparent", border: `1px solid ${hovered ? "#E8D5A3" : "transparent"}`, transition: "all .15s", boxShadow: hovered ? "0 8px 20px rgba(61,43,31,.08)" : "none", cursor: "pointer" }}
      >
                  ...
                  {(orders?.orders || []).map((o: any, i: number) => {
                    ...
                    return (
                      <div key={o._id} style={{ display: "grid", gridTemplateColumns: "1fr 120px 100px 90px", padding: "14px 22px", borderBottom: i < (orders?.orders?.length - 1) ? "1px solid #F5EDE0" : "none", alignItems: "center" }}>
                        ...
                        <div style={{ fontSize: "12px", color: "#6B4F12", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{o.buyerName}</div>
                        <div style={{ fontSize: "13px", fontWeight: "600", color: "#2C1810" }}>₹{o.total?.toLocaleString("en-IN")}</div>
                        <div style={{ display: "inline-flex", padding: "3px 8px", borderRadius: "99px", background: sc.bg, color: sc.color, fontSize: "10px", fontWeight: "700", letterSpacing: ".04em" }}>{o.status}</div>
                      </div>
                    );
                  })}
                </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div style={{ background: "#FFFDF9", border: "1px solid #E8D5A3", borderRadius: "16px", overflow: "hidden" }}>
              <div style={{ padding: "14px 18px", borderBottom: "1px solid #F0E4C0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h3 style={{ fontSize: "14px", fontWeight: "600", color: "#2C1810", fontFamily: "Georgia, serif" }}>Recent Transactions</h3>
              ...
              {loading ? (
                ...
                <div style={{ padding: "24px 18px", textAlign: "center" }}>
                  ...
                  <p style={{ fontSize: "12px", color: "#A08060" }}>No transactions yet</p>
                </div>
              ) : (
                <div style={{ padding: "8px 0" }}>
                  {transactions.map((tx, i) => (
                    <div key={tx._id} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 18px", borderBottom: i < transactions.length - 1 ? "1px solid #F5EDE0" : "none" }}>
