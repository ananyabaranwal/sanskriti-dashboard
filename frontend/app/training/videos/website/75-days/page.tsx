...
const VIDEOS = [
  { id:1, title:"Day 1 — Domain & Hosting Setup",          duration:"18:24", desc:"Register your domain, choose hosting, get your site live.", free:true  },
  { id:2, title:"Day 5 — Website Design Fundamentals",     duration:"24:10", desc:"Create a professional antique store that builds trust.", free:false },
  { id:3, title:"Day 10 — Product Photography for Web",    duration:"31:45", desc:"Shoot and edit product photos that sell.", free:false },
  { id:4, title:"Day 15 — Writing Product Descriptions",   duration:"22:08", desc:"SEO-friendly descriptions that convert browsers to buyers.", free:false },
  { id:5, title:"Day 20 — Payment Gateway Integration",    duration:"19:33", desc:"Set up Razorpay, UPI and COD on your website.", free:false },
  { id:6, title:"Day 30 — SEO for Antique Websites",       duration:"28:17", desc:"Rank on Google for antique-related searches.", free:false },
  { id:7, title:"Day 45 — Running Your First Ad Campaign", duration:"26:52", desc:"Facebook and Google ads for antique sellers.", free:false },
...
function VideoList({ videos, isLoggedIn, onLock }: { videos: typeof VIDEOS; isLoggedIn: boolean; onLock: () => void }) {
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:"10px" }}>
      {videos.map((v) => {
        const canWatch = isLoggedIn || v.free;
        return (
          <div key={v.id} onClick={() => !canWatch && onLock()} style={{ display:"flex",alignItems:"center",gap:"16px",padding:"16px 20px",borderRadius:"12px",border:`1.5px solid ${canWatch?"rgba(155,0,32,.1)":"#f0f0f0"}`,background:canWatch?"#fff":"#fafafa",cursor:canWatch?"default":"pointer",transition:"all .2s" }}>
...
export default function Page() {
  ...
  const lockedCount = VIDEOS.filter(v => !v.free).length;
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif",background:"#fff",minHeight:"100vh",paddingTop:"68px" }}>
      ...
      <div style={{ background:BURG,padding:"52px 48px 44px",position:"relative",overflow:"hidden" }}>
        ...
        <div style={{ maxWidth:"900px",margin:"0 auto",position:"relative" }}>
          ...
          <h1 style={{ fontFamily:"'Playfair Display',serif",fontSize:"clamp(28px,4vw,48px)",fontWeight:700,color:"#fff",marginBottom:"10px",lineHeight:1.1 }}>75 Days Website Training</h1>
          <p style={{ fontSize:"15px",color:"rgba(255,255,255,.7)",fontStyle:"italic",marginBottom:"12px",fontFamily:"'Playfair Display',serif" }}>From Zero to Your First Sale in 75 Days</p>
          <p style={{ fontSize:"14px",color:"rgba(255,255,255,.65)",lineHeight:1.75,maxWidth:"580px",fontFamily:"'DM Sans',sans-serif",marginBottom:"20px" }}>A complete day-by-day structured program to build, launch, and grow your antique website.</p>
      ...
      {!isLoggedIn && (
        <div style={{ background:"#fff3f4",borderBottom:"1px solid rgba(155,0,32,.15)",padding:"12px 48px" }}>
          <div style={{ maxWidth:"900px",margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",gap:"16px",flexWrap:"wrap" }}>
            <div style={{ fontSize:"13px",color:BURG,fontFamily:"'DM Sans',sans-serif" }}>🔒 <strong>{lockedCount} videos locked.</strong> Sign in to unlock.</div>
      ...
      <div style={{ maxWidth:"900px",margin:"0 auto",padding:"48px 48px 80px" }}>
        <VideoList videos={VIDEOS} isLoggedIn={isLoggedIn} onLock={() => setShowLogin(true)} />
        {!isLoggedIn && (
