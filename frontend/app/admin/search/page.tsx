  ...
  return (
    <div style={{ maxWidth: "800px" }}>
      <div style={{ marginBottom: "28px" }}>
        ...
        <h1 style={{ fontSize: "26px", fontFamily: "Georgia,serif", color: "#111", fontWeight: 400 }}>Global Search</h1>
      </div>
      ...
      <div style={{ position: "relative", marginBottom: "24px" }}>
        ...
        <input value={query} onChange={e => handleChange(e.target.value)} placeholder="Search orders, clients, invoices, products, phone numbers..." autoFocus
          style={{ width: "100%", padding: "16px 18px 16px 50px", borderRadius: "14px", border: "2px solid #f0f0f0", fontSize: "15px", color: "#111", background: "#fff", outline: "none", fontFamily: "inherit" }} />
        {loading && <div style={{ position: "absolute", right: "18px", top: "50%", transform: "translateY(-50%)", fontSize: "13px", color: "#888" }}>Searching...</div>}
      </div>

      {results && (
        <div>
          <div style={{ fontSize: "13px", color: "#888", marginBottom: "16px" }}>{String(totalResults)} results for &quot;{query}&quot;</div>
          {sections.map(s => {
            const data = (results[s.key]?.data || []) as any[];
            if (!data.length) return null;
            return (
              <div key={s.key} style={{ marginBottom: "20px" }}>
                <div style={{ fontSize: "11px", fontWeight: 700, color: "#888", letterSpacing: ".08em", textTransform: "uppercase", marginBottom: "8px" }}>
                ...
                <div style={{ background: "#fff", border: "1px solid #f0f0f0", borderRadius: "12px", overflow: "hidden" }}>
                  {data.map((item: any, i: number) => (
                    <div key={item._id || i} onClick={() => router.push(navMap[s.key])}
                      style={{ padding: "12px 16px", borderBottom: i < data.length - 1 ? "1px solid #f5f5f5" : "none", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}
                      onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = "#f0f0f0"}
                      onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = "transparent"}
                    >
                      <div>
                        <div style={{ fontSize: "13px", fontWeight: 600, color: "#111" }}>{item.orderNumber || item.invoiceNumber || item.name || "—"}</div>
                      ...
                      <div style={{ textAlign: "right" }}>
                        {(item.total || item.grandTotal || item.totalRevenue || item.sellingPrice) ? (
                          <div style={{ fontSize: "13px", fontWeight: 700, color: "#7a001a" }}>
                            ₹{Number(item.total || item.grandTotal || item.totalRevenue || item.sellingPrice).toLocaleString("en-IN")}
                          </div>
                        ) : null}
                        <div style={{ fontSize: "10px", color: "#888" }}>{item.status || item.paymentStatus || item.stockStatus || ""}</div>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {!results && !loading && !query && (
        <div style={{ textAlign: "center", padding: "60px 20px" }}>
          ...
          <p style={{ fontSize: "16px", fontWeight: 500, color: "#111", marginBottom: "8px" }}>Search anything</p>
          <p style={{ fontSize: "13px", color: "#888" }}>Client names · Order numbers · Invoice numbers · Phone numbers · Product names</p>
        </div>
      )}
